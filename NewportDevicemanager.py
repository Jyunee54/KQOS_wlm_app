...
class NewportDeviceManager:
    def __init__(self, device_keys, debug = False):
        ...
        if device_key in self.device_keys:
            ...
            if self.devices[device_key] is None:  # 장치가 이미 연결되지 않았다면
                try:
                    ...
                    print(f"{device_key} 장치 연결 시도 중")
                    device = newport(id=1, key=device_key, debug=self.debug)
                    device.connected = True
                    self.devices[device_key] = device  # 장치 연결 상태 설정
